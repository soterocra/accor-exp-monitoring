...
public class TelegramService implements ItemObserver {
    ...
    public void newItem(Item item) {
        ...
        userService.findAll().parallelStream()
                .forEach(user -> {
                    Log.info("Enviando mensagem para usuário: " + user);
                    var text = "<b>Resultado da Busca</b> \\u{1F50D} \\u{1F50D} \\u{1F50D} \\n\\n<b>Nome do Evento:</b> " + item.getName() + "\\n\\n<b>Pontos Necessários:</b> \\u{1F4B0} " + item.getPrice() + "\\n\\n<b>Link:</b> \\u{1F310} " + item.getLink() + "\n";
